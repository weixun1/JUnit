...
public class PlaylistTest extends TestCase {
    ...
    public void test_sortByArtist() {
    	...
    	Collections.sort(aPlaylist.getList(), new SongArtistComparator());


    	for(int i = 1; i< aPlaylist.size(); i++)
    	{
    		assertTrue(aPlaylist.getSong(i-1).getArtist().compareTo(aPlaylist.getSong(i).getArtist()) <= 0 );
    	}
    ...
    public void test_sortByTitle() {
    	...
    	Collections.sort(aPlaylist.getList(), new SongTitleComparator());
    	
    	for(int i = 1; i < aPlaylist.size(); i++)
    	{
    		assertTrue(aPlaylist.getSong(i - 1).getTitle().compareTo(aPlaylist.getSong(i).getTitle()) <= 0);
    	}
    }
}
